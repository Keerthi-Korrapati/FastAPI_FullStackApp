...
async def create_todo(request: Request, title: str = Form(...), description: str = Form(...),
    ...
    return RedirectResponse(url="/todos", status_code=status.HTTP_302_FOUND) # to redirect the client to a different endpoint
...
async def edit_todo(request: Request, todo_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("edit-todo.html", {"request": request, "todo": todo, "user": user}) # return HTML responses that are dynamically generated using template files.
...
async def test(request: Request):
    return templates.TemplateResponse("register.html", {"request": request}) # to return the response by giving html file, context i.e., request
